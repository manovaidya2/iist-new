...
const handleDelete = async (id) => {
                ...
                >
                  ...
                  <td className="p-3 text-center">
                    {item.uploadedSignature ? (
                      <img
                        src={`https://api.iisd.io/${item.uploadedSignature}`}
                        className="h-12 mx-auto rounded shadow"
                      />
                    ) : item.drawnSignature ? (
                      <img
                        src={`https://api.iisd.io/${item.drawnSignature}`}
                        className="h-12 mx-auto rounded shadow"
                      />
                    ) : (
  ...
  {selectedItem.idFile && (
    <div className="mt-3">
      ...
      selectedItem.idFile.endsWith(".jpeg") ? (
        <img
          src={`https://api.iisd.io/${selectedItem.idFile}`}
          className="h-28 mt-2 rounded shadow"
        />
      ) : (
        <a
          href={`https://api.iisd.io/${selectedItem.idFile}`}
          download
  ...
  {selectedItem.photoFile && (
    <div className="mt-3">
      ...
      <img
        src={`https://api.iisd.io/${selectedItem.photoFile}`}
        className="h-28 mt-2 rounded shadow"
      />
    </div>
  )}
  ...
  {selectedItem.experienceFile && (
    <div className="mt-3">
      ...
      <a
        href={`https://api.iisd.io/${selectedItem.experienceFile}`}
        download
        className="text-blue-600 underline"
      >
  ...
  <div className="mt-4">
    ...
    {selectedItem.uploadedSignature ? (
      <img
        src={`https://api.iisd.io/${selectedItem.uploadedSignature}`}
        className="h-24 mt-2 rounded shadow"
      />
    ) : selectedItem.drawnSignature ? (
      <img
        src={`https://api.iisd.io/${selectedItem.drawnSignature}`}
        className="h-24 mt-2 rounded shadow"
      />
    ) : (
      "No Signature"
